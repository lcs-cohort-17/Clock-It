(function () {
  ...
  function pageConfig() {
    ...
    return {
      loginApi:
        page && page.dataset.loginApi ? page.dataset.loginApi : "/api/login",
      forgotPasswordApi:
        page && page.dataset.forgotPasswordApi
          ? page.dataset.forgotPasswordApi
          : "/api/forgot-password",
      socialLoginApi:
        page && page.dataset.socialLoginApi
          ? page.dataset.socialLoginApi
          : "/api/social-login",
      adminRoute:
        page && page.dataset.adminRoute
          ? page.dataset.adminRoute
          : "/admin-dashboard",
      staffRoute:
        page && page.dataset.staffRoute
          ? page.dataset.staffRoute
          : "/staff-dashboard",
    };
  }
  ...
  window.clockitLogin = function () {
    return {
      ...
      validateLogin: function () {
        ...
        if (
          this.loginMethod === "employeeId" &&
          !String(this.employeeId || "").trim()
        ) {
      ...
      findMockUser: function () {
        ...
        var employeeId = String(this.employeeId || "")
          .trim()
          .toLowerCase();
        ...
        for (var i = 0; i < users.length; i += 1) {
          ...
          if (
            ...
            String(user.password || "") === String(this.password || "")
          ) {
            return user;
          }
      ...
      redirectForUser: function (user, fallbackRedirect) {
        ...
        var target =
          fallbackRedirect ||
          (role === "admin" ? config.adminRoute : config.staffRoute);
      ...
      socialLogin: async function (provider) {
        ...
        var picked = prompt(
          "Demo social login - enter the email for " + provider + " account:",
        );
        if (!picked) return;

        try {
          var result = await postJson(pageConfig().socialLoginApi, {
            provider: provider,
            email: normalizeEmail(picked),
          });
          this.redirectForUser({ role: result.role }, result.redirect);
        } catch (err) {
