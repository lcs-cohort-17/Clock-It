...
if (isset($_GET['logout']) && $_GET['logout'] == 1) {
    logoutEmployee();
    setLoggedInEmployee('EMP001');
    header('Location: index.php');
    exit();
}

// Handle employee switching from dropdown
if (isset($_POST['switch_employee']) && !empty($_POST['switch_employee'])) {
    setLoggedInEmployee($_POST['switch_employee']);
    header('Location: index.php');
    exit();
}

// For demo purposes, if no employee is logged in, default to first employee
if (!getLoggedInEmployee()) {
    setLoggedInEmployee('EMP001'); // Shaheed Karlie
}

$loggedInEmployee = getLoggedInEmployee();

if (!$loggedInEmployee) {
    $employees = getEmployeeList();
    $loggedInEmployee = $employees[0];
    setLoggedInEmployee($loggedInEmployee['id']);
}

// Get all employees for the switcher dropdown
$allEmployees = getEmployeeList();
...
<head>
    ...
    <title>My Attendance Dashboard - <?php echo htmlspecialchars($loggedInEmployee['name']); ?></title>
...
<body class="bg-[#EEF3F8] font-sans antialiased" 
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                <div class="flex items-center">
                    ...
                    <div class="ml-10">
                        <a href="index.php" class="px-3 py-2 rounded-md text-sm font-medium text-gray-900 bg-gray-100">My Dashboard</a>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <!-- Employee Switcher Dropdown -->
                    <div class="employee-switcher" x-data="{ open: false }">
                        <button @click="open = !open" class="flex items-center gap-2 px-3 py-1.5 bg-amber-50 border border-amber-200 rounded-full hover:bg-amber-100 transition-all">
                            <svg class="w-4 h-4 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path>
                            </svg>
                            <span class="text-sm text-amber-700">Switch Employee</span>
                            <span class="demo-badge">Demo</span>
                            <svg class="w-3 h-3 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" @click.away="open = false" class="employee-dropdown" x-cloak>
                            <div class="py-2">
                                <div class="px-3 py-2 text-xs font-semibold text-gray-500 border-b">Select Employee</div>
                                <?php foreach ($allEmployees as $emp): ?>
                                <form method="POST" class="block">
                                    <input type="hidden" name="switch_employee" value="<?php echo $emp['id']; ?>">
                                    <button type="submit" class="w-full text-left px-3 py-2 hover:bg-gray-50 transition-colors <?php echo ($loggedInEmployee['id'] == $emp['id']) ? 'bg-blue-50' : ''; ?>">
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 bg-gradient-to-br from-blue-600 to-blue-700 rounded-full flex items-center justify-center text-white font-bold text-xs">
                                                <?php echo $emp['avatar']; ?>
                                            </div>
                                            <div>
                                                <p class="text-sm font-medium text-gray-900"><?php echo $emp['name']; ?></p>
                                                <p class="text-xs text-gray-500"><?php echo $emp['id']; ?> • <?php echo $emp['dept']; ?></p>
                                            </div>
                                            <?php if ($loggedInEmployee['id'] == $emp['id']): ?>
                    ...
                    <div class="hidden md:flex items-center gap-2 px-3 py-1.5 bg-gray-100 rounded-full">
                        ...
                        <span class="text-sm text-gray-600"><?php echo htmlspecialchars($loggedInEmployee['name']); ?></span>
                    </div>
                    <a href="?logout=1" class="flex items-center gap-1.5 px-3 py-1.5 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-all border border-transparent hover:border-red-200">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <h1 class="mt-2 text-4xl font-bold text-white md:text-5xl">My Attendance</h1>
                    <p class="mt-2 text-base text-blue-100">View your personal attendance history and insights</p>
                </div>
                ...
                <div class="view-toggle-container">
                    <button @click="setView('list')" 
                            ...
                            class="view-toggle-btn flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
            <div class="bg-white rounded-2xl shadow-sm p-4 border-2 border-gray-200 stat-card">
                ...
                <p class="text-2xl font-bold text-slate-800 mt-1" x-text="summary.total">0</p>
            </div>
            <div class="bg-white rounded-2xl shadow-sm p-4 border-2 border-gray-200 stat-card">
                    ...
                    <template x-for="(day, index) in calendarDays" :key="index">
                        <div :class="{'calendar-day-cell': true, 'prev-month': day.isPrevMonth, 'next-month': day.isNextMonth}">
                            <div class="flex justify-between items-start">
                                ...
                                <span x-show="day.records.length > 0" class="text-xs font-medium text-blue-600 bg-blue-100 px-2 py-0.5 rounded-full" x-text="day.records.length"></span>
                            </div>
    ...
    <!-- Footer -->
    <footer class="mt-8 py-6 text-center border-t border-gray-200 bg-white">
        <p class="text-xs text-gray-500">&copy; 2026 Attendance Management System. All rights reserved.</p>
        <p class="text-xs text-gray-400 mt-1">Logged in as: <?php echo htmlspecialchars($loggedInEmployee['name']); ?> (<?php echo htmlspecialchars($loggedInEmployee['id']); ?>)</p>
    </footer>
            ...
            return {
                ...
                updateCalendar() {
                    ...
                    for (let i = startingDayOfWeek - 1; i >= 0; i--) {
                        ...
                        const dayRecords = this.filteredData.filter(record => record.date === dateStr);
                        days.push({
                            date: dateStr,
