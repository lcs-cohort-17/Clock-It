...
?>
<section class="card dashboard-card onsite-card h-100 rounded-4 border shadow-sm bg-white">
  <div class="card-header dashboard-card-header bg-white border-0 p-4 pb-0">
    <div>
  ...
  <div class="card-body dashboard-card-body onsite-card-body p-4">
    ...
    <div class="empty-state onsite-empty-state" x-show="!loading && onsiteStaff.length === 0">
      ...
      <template x-for="staff in onsiteStaff" :key="staff.id || staff.name">
        <div class="list-group-item px-0 py-3 border-0 border-bottom">
          <div class="d-flex align-items-center justify-content-between gap-3">
            ...
            <div class="min-width-0">
              <p class="staff-time text-muted small mb-0 text-end" x-text="`Signed in: ${staff.signed_in_at || staff.signedInAt}`"></p>
            </div>
          </div>
        </div>
      </template>
