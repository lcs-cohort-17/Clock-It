...
describe("stats controller - cache warm", () => {
  it("does not call Supabase on a second request within the TTL", async () => {
    ...
    const supabase = makeSupabaseMock([
      ...DEFAULT_COUNTS,
      // These should never be used while the cache is still warm(valid and not expired data)
      { count: 99, error: null },
      { count: 99, error: null },
      { count: 99, error: null },
      { count: 99, error: null },
  ...
  it("serves cached data up to but not including the 5 s TTL boundary", async () => {
    ...
    await request(app).get("/api/admin/dashboard/stats");
    vi.advanceTimersByTime(1000); // Bypass rate limiter
    vi.advanceTimersByTime(3999); // Still 1 ms before the 5 s TTL
    await request(app).get("/api/admin/dashboard/stats");

    expect(supabase.from).toHaveBeenCalledTimes(4); // only the first request hit Supabase
  });
});
